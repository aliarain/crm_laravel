                ...
                <div class="row p-0">
                    <div class="col-md-12">
                        <form action="{{ $data['url'] }}" method="post">
                            ...
                            <div class="form-group mb-3">
                                <label class="form-label">{{ _trans('performance.Competence Type') }} <span class="text-danger">*</span></label>
                                <select name="competence_type_id" class="form-control modal_select2" required>
                                    @foreach ($data['competence_types'] as $competence_type)
                                        <option {{ @$data['edit']->competence_type_id == $competence_type->id ? 'selected' : '' }}  value="{{ $competence_type->id }}">{{ $competence_type->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group text-right d-flex justify-content-end">
                                <button type="submit"
                                    class="crm_theme_btn pull-right">{{ @$data['button'] }}</button>
                            </div>
                        </form>
